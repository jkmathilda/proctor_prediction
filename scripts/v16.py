...
def main(args):
    ...
    # v16 vs v10: drop hyd_cond_hyd_gradient outright -- not imputed, not used
...
def parse_args():
    ...
    p.add_argument("--out", default=str(repo_root / "submissions" / "submission_v16_gpr+mice_nohydgrad_3split.csv"))
    p.add_argument("--model_out", default=str(repo_root / "models" / "v16_gpr+mice_nohydgrad_3split.pt"),
                   help="path to save the fitted GP model as a .pt file (empty to skip)")
    p.add_argument("--uncertainty_out", default=str(repo_root / "submissions" / "v16_uncertainty_3split.csv"),
                   help="CSV of per-sample predictive std (empty string to skip)")
    p.add_argument("--report_dir", default=str(repo_root / "figures"))
    p.add_argument("--log", default=str(repo_root / "docs" / "v16_logs"/"v16_run.log"))
